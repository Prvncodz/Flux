...
const getUserPlaylists = asyncHandler(async (req, res) => {
    ...
    throw new ApiError(400,"invlaid user id")
      ...
      throw new ApiError(400,"page number invalid")                                               }                                                    if(isNaN(limitNum)|| limitNum<1){            throw new ApiError(400,"limit is invalid")
  }
     const AllPlaylistsByUser= await Playlist
     .find({owner:userId})
     .skip(skipNum)
     .limit(limitNum)
...
const getPlaylistById = asyncHandler(async (req, res) => {
     ...
     if(!isValidObjectId(playlistId)){              throw new ApiError(400,"invlaid id")
     }
...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
  ...
  if(!addVideoToPlaylist){
    throw new ApiError("unable to add video yo the playlist")
  }
  return res
  .status(200)
  .json(new ApiResponse(
...
const removeVideoFromPlaylist = asyncHandler(async (req, res) => {
      ...
      if(!isValidObjectId(playlistId)||!isValidObjectId(videoId)){                                  throw new ApiError(400,"inavlid id")            }
...
const deletePlaylist = asyncHandler(async (req, res) => {
  ...
  .json(new ApiResponse(
      200,
      {},
      "deleted this Playlist successfully"
    ))
})
