export default function CreateComponent({ handleUpload }) {
  return (
    <div className="absolute bg-gray-50 shadow-sm rounded-xl h-auto w-auto top-12 z-10 right-0 p-3 transition-all pop-in delay-75 flex flex-col gap-2">
      <div className="flex items-center gap-3 w-55  p-3 rounded-lg  hover:bg-gray-100" onClick={() => handleUpload("video")}>
        <svg
      ...
      <div className="flex items-center gap-3 w-55 p-3 rounded-lg hover:bg-gray-100" onClick={() => handleUpload("post")}>
        <svg
          width="30" height="30" viewBox="0 0 24 24"
          fill="none" stroke="#111" strokeWidth="1.5"
          strokeLinecap="round" strokeLinejoin="round"
