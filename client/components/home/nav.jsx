		...
		<nav className="h-13 flex justify-between items-center bg-gray-50 w-full">
			...
			</div>
			{
				isLogged && notLoggedOut? (
					<>
						<div className="m-2 w-12 h-9 flex justify-center relative items-center" onClick={() => { isActive?setIsActive(false):setIsActive(true)}}>
							<img src={user?.avatar?.url} className="h-10 w-10 rounded-full" loading="lazy" />
						</div>
						{isActive && <>
							<div className="absolute top-15 right-3 bg-gray-50 border border-1 border-gray-600 border border-1 border-gray-200 h-auto w-50 rounded-xl flex flex-col overflow-hidden">
								...
								<div className=" relative flex justify-left items-center w-full mb-3">
									<img src={user?.avatar?.url} className="h-11 w-11 rounded-full relative mr-3 ml-2 left-0" loading="lazy" />
									<span>
									<div className=" font-normal text-lg text-gray-700 w-auto text-left">{user?.fullName}</div>
									<div className="text-gray-600 font-normal text-lg text-gray-600 w-auto -mt-1 text-left">{"@"+user?.userName}</div>
									</span>
								</div>
								...
								<div onClick={handleSignout} className=" flex justify-center"><svg className=" h-6 w-5 " xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
...
</svg><span className="ml-2 text-medium font-medium">Sign Out</span></div>
								</div>
							</div>
						</>}
					</>
