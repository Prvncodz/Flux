...
export default function CommentFeed({ fetchType, Id, isOpen, setIsOpen, setShowSignInPopup }) {
	...
	useEffect(() => {
		...
		async function fetchAllCommentsOnTweet(id) {
			...
			try {
				await axios
					.get(
						`/comments/${id}/get-tweet-comments?page=${page}${user?._id ? `&userId=${user._id}` : ``}`, { signal }
					)
					.then((res) => {
						if (res.data.data.length == 0) {
							setHasNoMore(true);
		...
		async function fetchAllCommentsOnComment(id) {
			...
			try {
				await axios
					.get(
						`/comments/${id}/get-comment-comments?page=${page}${user?._id ? `&userId=${user._id}` : ``}`, { signal }
					)
					.then((res) => {
			...
			>
				...
				{areCommentsFetched && isOpen ? (
					<div className="h-full w-full" ref={ref}>
						...
						/>
						<AddCommentsBox Id={Id} fetchType={fetchType} className={"mb-8"} setShowSignInPopup={setShowSignInPopup} />
						{comments.map((comment, idx) => (
							<CommentComponent key={idx} comment={comment} setShowSignInPopup={setShowSignInPopup} idx={idx} commentsLength={comments.length} setLoading={setLoading} />
						))}
					</div>
